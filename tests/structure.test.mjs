...
test("separate Azure profile front doors are packaged without account data", async () => {
  const files = [
    ["bin", "az-work"],
    ["bin", "az-devops"],
    ["bin", "az-devops.py"]
  ...
  assert.doesNotMatch(source, /(?:username|email)\s*[:=]\s*["'][^"']+@[^"']+["']/i);
  assert.match(source, /AZURE_DEVOPS_EXT_PAT/);
  assert.match(source, /acquire_token_silent_with_error/);

  const example = await json("examples", "profiles.json");
  assert.equal(example.devops.authority, "https://login.microsoftonline.com/organizations");
  assert.equal(example.devops.config_dir, "~/.azure-devops-personal");
});
